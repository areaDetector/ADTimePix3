...
* **`asynPortDriver:getParamStatus: port=TPX3 error setting parameter 51 in list 5, invalid list`** (or parameter 52, 50, etc.): **Fixed in R1-6.** The message means asyn was asked for parameter 51 (ARRAY_DATA) at "list 5" (address 5) on port TPX3, but address 5 did not exist. The driver was built with `maxAddr=4` (valid addresses 0–3), while the histogram channel uses NDArray address 5. In R1-6 the driver was changed to use `maxAddr=6`, so address 5 is valid and this warning should no longer appear. **Root cause (from asynReport):** "Parameter 51 is undefined, name=ARRAY_DATA" in "list 5" referred to the *address* (list 5 = address 5) being invalid on the TPX3 port, not to parameter 51 itself. **Why did the parameter number vary (51 vs 52)?** Different builds (e.g. Ubuntu 22.04 vs Red Hat 9) can have different ADCore/asyn parameter order, so the reported parameter index can differ; the underlying issue was always the invalid address. **No action required** if you are on R1-6 or later. If you still see this on an older build, recompile with the updated driver so that `maxAddr=6` is used.
...
* **`asynPortDriver:getParamStatus: port=TPX3 error setting parameter 51 in list 5, invalid list`**: Fixed in R1-6. The driver was created with `maxAddr=4` (addresses 0–3 only), but the histogram channel uses NDArray address 5. When asyn or plugins accessed parameter 51 (ARRAY_DATA) at address 5, asyn reported "invalid list" because that address did not exist. The driver now uses `maxAddr=6` so that PrvImg=0, Img=1, and PrvHst=5 are all valid. **No action required** after recompile and IOC restart.
