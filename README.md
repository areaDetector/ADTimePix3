...
* **`asynPortDriver:getParamStatus: port=TPX3 error setting parameter 51 in list 5, invalid list`**: This warning appears when histogram data callbacks are triggered for the `PrvHstHistogramFrame` PV (parameter 51, `ADTimePixPrvHstHistogramFrame`). It is harmless and can be safely ignored. The warning occurs because asyn's internal alarm/status information arrays for Int32Array parameters (list 5 in asyn's internal structure) are not fully initialized for parameters with high indices (parameter 51 in this case). When `doCallbacksInt32Array()` is called for this parameter, asyn automatically tries to update alarm/status information via `getParamStatus()`, `getParamAlarmStatus()`, and `getParamAlarmSeverity()`, but these functions fail because the alarm/status arrays for Int32Array parameters aren't properly sized for parameter index 51. The data callbacks complete successfully and histogram data is transmitted correctly to EPICS PVs - only the alarm/status metadata update fails. This is a known asyn library limitation when drivers have many parameters (ADTimePix3 has ~237 parameters). The standalone histogram IOC doesn't show this warning because it has fewer parameters (~33), so array parameters are registered earlier with lower indices where alarm/status arrays are properly initialized. **Why only this PV?** Parameter 51 (`ADTimePixPrvHstHistogramFrame`) is the only Int32Array parameter with a high enough index to trigger this issue. Other array parameters (Int64Array, Float64Array) use different internal lists and don't show this warning. **Is this fixed in newer asyn versions?** No - this is a persistent limitation in asyn library across versions. The alarm/status arrays for array parameters are sized based on the number of parameters, but for drivers with many parameters, high-index array parameters may exceed the allocated size. **Can this be fixed by moving the parameter to a lower index?** Potentially yes - moving `ADTimePixPrvHstHistogramFrame` to be created earlier (e.g., right after `ADTimePixImgImageFrame` at parameter index ~48) would give it a lower index and might avoid the warning. However, this requires reordering both the parameter declaration in `ADTimePix.h` and the `createParam()` call in `ADTimePix.cpp`, which could affect parameter numbering for other parameters. **No action required** - the functionality works correctly despite the warning. The warnings can be filtered from log files if desired, but they don't affect driver operation.
