...
* **`DataType_RBV` showing "Illegal Value" when histogram channel is enabled**: This issue has been fixed in the driver. The histogram channel uses `NDInt64` data type (value 8) for accumulated histogram data, but the `DataType_RBV` database record enum only supports values 0-5 (UInt8, Int8, UInt16, Int16, UInt32, Int32). The driver now preserves the previous `NDDataType` value and does not set it to `NDInt64` for histogram data, preventing the "Illegal Value" status. The NDArray itself still correctly uses `NDInt64` data type internally, which is what matters for data processing and callbacks. **No action required** - this has been resolved in the code.
