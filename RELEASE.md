...
* **Family-safe BPC mask semantics**:
  ...
  * MPX3 mask readback, counts, writes, and export operate on one **big-endian 16-bit word per pixel**. Bit 0 masks both counters; bits 1–5 and 6–10 are the two adjustment fields; bit 11 is test-pulse injection. ASI identifies bits 12–15 as unused and normally zero; the driver still preserves them defensively.
  * TPX3 bits 6–7 are likewise unused and normally zero. All mask changes preserve every non-mask bit rather than normalizing unused fields.
  * MPX3 mask images and JSON use Serval's explicit per-chip `Chip`, `X`, `Y`, and `Orientation` layout and fail closed when that layout is missing or unsupported. All eight Serval chip-orientation strings are supported, and a detector-orientation write refreshes `Layout.Rotated.Chips` before later mask operations.
  * An asymmetric live operator-mask test exposed two independent layout conventions: the four non-axis-swapped direction strings must be applied literally, and Serval tile `Y` origins are bottom-based while areaDetector image Y is top-based. The shared mapping converts `image_tile_y = image_height - chip_width - serval_y`. After rebuilding, the L-shaped mask aligned with both threshold-counter images in all eight global detector orientations. These live cases exercise all eight Serval chip-orientation strings.
  * Added deterministic family, packed-value, classification, count, bounds, and bit-preservation tests in **`servalProtocolFixtureTest`**.

* **MPX3 packed PixelConfig layout**:
  * Correct the MPX3 model from two concatenated 64 KiB byte slices to one **big-endian 16-bit word per pixel**: mask bit 0, threshold-0 adjustment bits 1–5, and threshold-1 adjustment bits 6–10. The per-chip size remains **131072 bytes**.
...
* **PixelConfig vs on-disk BPC (on-demand compare)**: Documents the per-chip configuration **held and returned by SERVAL** (GET `/detector/chips/<i>/PixelConfig`) versus the **binary file** the IOC points at (`BPCFilePath` + `BPCFileName`). ASI indicates this value is converted from the BPC rather than read directly from detector registers, so a match does not independently prove hardware register state. Full operator-oriented explanation: **`documentation/PIXELCONFIG_BPC_DIFF.md`**.
  ...
  * **Waveform `PixelConfigDiff`**: After each refresh, holds **|SERVAL byte - BPC byte|** at each TPX3 image pixel **`(i,j)`** using **`k = pelIndex(i,j)`** (same mapping as mask write / `maskCircle`), **not** `bpc2ImgIndex`--those two were not equivalent for all quad orientations (e.g. **LEFT**), which broke alignment at chip boundaries. Same `NELEMENTS` as `BPC` / `MaskBPC` in `MaskBPC.template`. Phoebus: `Mask/Mask.bob` embeds `PixelConfigMaskPanel.bob` (refresh, table, 512x512 / 256x256 |Delta| heatmaps).
  * **`MaskBPC` read from `.bpc`**: Loading the mask image from disk uses **`pelIndex(i,j)`** and **`j*COLS+i`** (clears **`value[]`** first), matching mask **write** and **`PixelConfigDiff`**; **`bpc2ImgIndex`** is no longer used on that path.
  ...
  * **Mask mapping bug fix**: `bpc2ImgIndex` quad-chip index used **`(bpcIndex+1)/chipPelCount`**, which misclassified the last pel of each 64 KiB block and the final file byte (`chip == 4`), producing bogus **"detector larger than 2x2"** console messages and wrong boundary mapping. Now **`bpcIndex / chipPelCount`** (for remaining **`bpc2ImgIndex`** callers).
...
* **Masked pels JSON export (`RefreshPixelConfig`)**: When a BPC file is read successfully during **`RefreshPixelConfig`**, the driver writes **`<BPCFilePath><stem>_masked_pels.json`** (stem = **`BPCFileName`** with a trailing **`.bpc`** removed, case-insensitive) containing **`format_version`**, **`source`**, optional **`detector`/`acquisition`**, **`masked_pels`** (BPC index, chip, `lx`/`ly`, image **`i`/`j`** via **`bpc2ImgIndex`**), and **`Bad pixels`** (median 1x1) for **NDPluginBadPixel**. Readbacks: **`TPX3_MASKED_PELS_JSON_RBV`**, **`TPX3_MASKED_PELS_COUNT_RBV`**, **`TPX3_MASKED_PELS_EXPORT_STATUS_RBV`** (`File.template` as **`MaskedPelsJson_RBV`**, etc.). Phoebus: **`op/bob/Mask/PixelConfigMaskPanel.bob`** shows count / status / path; **`Mask.bob`** embedded panel height updated to **920** px. See **`documentation/MASKED_PIXELS_JSON_AND_STREAMING.md`**.
...
* **Processed Img file saving (Option A)**: The driver can push the current image accumulation (running sum and sum-of-N) as NDArrays to file plugins so that `ImgImageData` and `ImgImageSumNFrames` can be saved as TIFF or HDF5.
  ...
  * **NeXus-style file output**: IOC boot examples `iocs/tpx3IOC/iocBoot/iocTimePix/templates/hdf5/hdf5_minimal.xml` (NDFileHDF5 `hdf5_layout`) and `iocs/tpx3IOC/iocBoot/iocTimePix/templates/nexus/plugin_template.xml` (NDFileNexus `NXroot` template, validated with ADCore `XML_schema/template.sch`). README documents that HDF5 and Nexus plugins use different XML dialects, template PVs, and `TemplateFilePath` concatenation rules.
...
* **PrvHst NeXus/HDF5 1D ToF example layout**: Added `iocs/tpx3IOC/iocBoot/iocTimePix/templates/hdf5/hdf5_prvhst_histogram.xml` (`hdf5_layout`) for NDFileHDF5. The main detector dataset stores counts (address 5 by default), with uniform ToF axis metadata from NDAttributes (`PrvHstTimeBin0Ms`, `PrvHstTimeBinStepMs`, `PrvHstNumBins`) so clients can reconstruct bin centers without requiring a second full axis dataset.
