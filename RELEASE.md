...
* **Driver logging (asyn)**: Internal `ERR` / `WARN` / `LOG` / `FLOW` helpers use a single `ADTPX3_FUNC` macro: on GCC/Clang it defaults to `__PRETTY_FUNCTION__` (richer context); other compilers use `__func__`. For shorter log lines, build with `-DADTPX3_LOG_SHORT` (see commented line in `tpx3App/src/Makefile`). Variadic `*_ARGS` macros use `##__VA_ARGS__` so format-only calls (no extra printf args) compile. `WARN` / `WARN_ARGS` use `ASYN_TRACE_WARNING`; the port trace mask must include the warning bit to see them (they are not tied to ERROR-level trace).
