...
* **Clean Shutdown for TCP Streaming**: Fixed "Broken pipe" errors when stopping acquisition with preview and/or image channels enabled. Shutdown sequence now properly stops Serval measurement first, waits for Serval's TcpSender threads to stop sending, then signals worker threads to exit. This ensures clean shutdown even with rate mismatches (e.g., 60 Hz frame rate vs 5 Hz preview rate). Worker threads now detect "Connection closed by peer" as expected behavior during shutdown.
