...
* **CONNECT/DISCONNECT (graceful reconnection without IOC restart)**: The driver now supports connection status monitoring and optional config refresh when SERVAL or the detector reconnects, so the IOC does not need to be restarted when the detector/emulator or SERVAL restarts.
  ...
  * **Lightweight connection check**: New `checkConnection()` performs a lightweight GET to SERVAL and `/dashboard` and updates only `ServalConnected_RBV`, `DetConnected_RBV`, `ADStatusMessage`, and (when connected) `DetType`/`ADModel`. It does not call `getServer()` or full `getDashboard()`.
  ...
  * **Behavior on reconnect**: Status PVs are set to connected, `ADStatusMessage` is cleared to "OK", and channel config is refreshed via `getServer()` once. Detector initialization (BPC/DACS load, `initCamera()`) is not run on reconnect.
