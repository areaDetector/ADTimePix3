...
- **Phoebus:** `TimePix3Detector.bob` embeds eight chip panels in a **4×2** grid (geometry maintained in the BOB). See also `DetectorConfig.bob` (Vth), mask / accumulation BOBs for **1024×512** at `PixCount_RBV == 524288`, and **`Detector/TimePixDetectorVoltages.bob`** (Display Builder under **`op/bob/Detector`**) or **`TimePixDetectorVoltages.opi`** (legacy BOY under **`op/opi/Detector`**) for **Pwr3–Pwr5**. **`TimePixDetectorHealth.bob`** summarizes health and cross-links to voltages.
